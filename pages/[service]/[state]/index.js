  ...
  return (
    <div className="text-center">
      <Head>
        ...
        <link
          rel="canonical"
          href={`https://${BASE_URL}/${service}/${state}`}
        />
      ...
      <ZipCity
        ...
        calenderHeading={
          state.replaceAll("-", " ") + " " + service.replaceAll("-", " ")
        }
        serving={state.replaceAll("-", " ")}
      />
      <Services services={services} />
      <ServiceInArea
        title={middleTextTitle
          .replaceAll("##service##", service.replaceAll("-", " "))
          .replaceAll("##area##", state.replaceAll("-", " "))}
        data={middletext}
        phone={phone}
      />
      <CallToActionCard
      ...
      />
      <Grid
        title={`${service.replaceAll("-", " ")} in ${state.replaceAll(
          "-",
          " "
        )}`}
        data={gridData}
...
export async function getStaticProps(context) {
  ...
  // Meta State Title
  const _meta_state_title = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=meta_state_title&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const meta_state_title = await _meta_state_title.json();

  // Meta State Description
  const _meta_state_description = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=meta_state_description&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const meta_state_description = await _meta_state_description.json();
  ...
  const _middleTextTitle = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=state_text_block_title&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const middleTextTitle = await _middleTextTitle.json();
  const _middletext = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/taglist?pattern=state_text_block_description&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const middletext = await _middletext.json();

  //   counties api
  const response = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/counties?state_name=${_state
      .toLowerCase()
  ...
  const _map = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/search_geo_phone?_sk=state:${params["state"]}`
  );
  ...
  if (data["list"].length == 0) {
    ...
    for (let i = 0; i < data["list"].length; i++) {
      let _county = data["list"][i].toLowerCase().replaceAll(" ", "-");
      let obj = {
    ...
    return {
      props: {
        ...
        meta_title: meta_state_title["text"],
        meta_description: meta_state_description["text"],
