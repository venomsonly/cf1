...
export default function City({
  ...
  const { service, state, county, city } = router.query;
  ...
  return (
    <div className="text-center">
      <Head>
        ...
        <link
          rel="canonical"
          href={`https://${BASE_URL}/${service}/${state}/${county}/${city}`}
        />
      ...
      <ZipCity
        ...
        area={city.replaceAll("-", " ")}
        calenderHeading={
          city.replaceAll("-", " ") + " " + service.replaceAll("-", " ")
        }
        serving={county.replaceAll("-", " ") + ", " + city.replaceAll("-", " ")}
        breadcrumbs={[
          {
      ...
      <ServiceInArea
        title={middleTextTitle
          .replaceAll("##service##", service.replaceAll("-", " "))
          .replaceAll("##area##", city.replaceAll("-", " "))}
        data={middletext}
        phone={phone}
      />
      <CallToActionCard
      ...
      />
      <Grid
        title={`${service.replaceAll("-", " ")} in ${city.replaceAll(
          "-",
          " "
        )}`}
        data={gridData}
...
export async function getStaticProps(context) {
  ...
  // Meta city Title
  const _meta_city_title = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=meta_city_title&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const meta_city_title = await _meta_city_title.json();

  // Meta city Description
  const _meta_city_description = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=meta_city_description&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const meta_city_description = await _meta_city_description.json();
  ...
  const _map = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/search_geo_phone?_sk=city:${params["city"]}`
  );
  ...
  const _middleTextTitle = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/tagtext?tag=city_text_block_title&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  const middleTextTitle = await _middleTextTitle.json();
  const _middletext = await fetch(
    `http://${process.env["NEXT_PUBLIC_API"]}/taglist?pattern=city_text_block_description&pass=${process.env["NEXT_PUBLIC_API_TOKEN"]}`
  );
  ...
  if (data["list"].length == 0) {
    ...
    return {
      props: {
        ...
        meta_title: meta_city_title["text"],
        meta_description: meta_city_description["text"],
